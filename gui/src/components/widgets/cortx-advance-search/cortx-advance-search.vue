  ...
  <div class="main-container">
    <v-row>
      ...
      <v-col class="pl-0">
        ...
        >
          <img src="@/assets/filter-new.svg" alt />
        </v-btn>
      </v-col>
    </v-row>
...
export default class CortxAdvanceSearch extends Vue {
  @Prop({ required: true }) config: AdvanceSearchConfig;
  ...
  toggleAdvanceSearch() {
    if (this.expand) {
      this.config.advanceForm?.forEach(ele => {
        const selectedElement = this.searchList.find(el => el.key == ele.name);
  ...
  primaryAction() {
    ...
    this.config.advanceForm?.forEach(ele => {
      if (ele.required)
        this.searchList.push({ key: ele.name, value: ele.value });
      else ele.value = undefined;
    });
  ...
  emitSearch() {
    this.searchList = [];
    this.config.advanceForm?.forEach(ele => {
      if (ele.value) this.searchList.push({ key: ele.name, value: ele.value });
    });
    this.$emit("filter-click", this.searchList);
  }
}
